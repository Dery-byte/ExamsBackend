...
/**
 * LLM strategy backed by the DeepSeek API.
 * DeepSeek exposes an OpenAI-compatible Chat Completions endpoint, so we reuse the
 * GPT DTOs ({@link GPTRequest}/{@link GPTResponse}).
 * <p>
 * Model is configurable via {@code deepseek.model} (default: deepseek-chat).
 */
@Service
public class DeepSeekEvaluationStrategy implements LLMEvaluationStrategy {

    private static final Logger log = LoggerFactory.getLogger(DeepSeekEvaluationStrategy.class);
    ...
    public QuizEvaluationResponse evaluate(GeminiRequest request, User user) {
        ...
        log.info("[DeepSeek] Evaluating {} questions for user {} with model {}",
        ...
        return helper.buildResponse(results, submissions, user, "DEEPSEEK");
    }
    ...
    private QuestionEvaluationResult evaluateOne(QuestionSubmission s) {
        ...
        while (attempt < MAX_RETRIES) {
            try {
                ...
                attempt++;
                log.error("[DeepSeek] HTTP error attempt {}/{}: {} — {}",
                ...
                attempt++;
                log.error("[DeepSeek] Error attempt {}/{}: {}", attempt, MAX_RETRIES, e.getMessage(), e);
                if (attempt >= MAX_RETRIES) return helper.createFailedResult(s, MAX_RETRIES, e);
